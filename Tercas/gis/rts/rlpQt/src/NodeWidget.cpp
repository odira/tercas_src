...
void NodeWidget::update()
{
    ...
    if (m_radioButton1->isChecked())
    {
        queryString = QString("SELECT nodepid, St_AsLatLonText(nodegeog::geometry,'D.DDDDDDDDDD'), St_SRID(nodegeog::geometry), nodenoteru "
                              "FROM rts.vw_rlp "
                              "WHERE nodepid=%1")
                .arg(m_nodepid);
        QSqlQuery query(queryString);
    ...
    {
        queryString = QString("SELECT nodepid, St_AsLatLonText(nodegeog::geometry, 'D M S.SSS'), St_SRID(nodegeog::geometry), nodenoteru "
                              "FROM rts.vw_rlp "
                              "WHERE nodepid=%1")
                .arg(m_nodepid);
        QSqlQuery query(queryString);
