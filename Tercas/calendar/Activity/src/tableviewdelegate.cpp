...
void TableViewDelegate::paint(QPainter *painter, const QStyleOptionViewItem &option, const QModelIndex &index) const
{
    ...
    if (col == ActivityModel::Columns::pid)
    {
//        QSqlTableModel *model = qobject_cast<QSqlTableModel*>(index.model());
//        QModelIndex indx = model->index(row, activity_color);
//        QString colorStr = model->data(indx).toString();
//        qDebug() << colorStr;

        qDebug() << index.data();
        QString abbrStr = index.model()->data(index.model()->index(row, ActivityModel::Columns::color)).toString();
